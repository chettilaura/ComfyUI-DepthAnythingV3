...
**What changed, and why:** the model loader (`nodes/nodes_loader.py`) was rewritten to:
...
- Use `accelerate`'s `init_empty_weights` + `set_module_tensor_to_device` to materialize weights directly on the target device/dtype during loading (meta-device trick), avoiding a redundant CPU copy of the full model — useful for the larger ViT-G variants.
