...
"""
    ...
    def _get_local_da3_model_path(self):
        specific_model_path = os.environ.get("COMFY_MODEL_TYPE_DEPTHANYTHING3")
    ...
    def loadmodel(self, model, precision="auto"):
        ...
        # ---- PHOTON ----
        local_da3_path = self._get_local_da3_model_path()

        # Se la variabile d'ambiente non è settata, usiamo un fallback standard
        # o puoi forzare un errore se preferisci.
        if not local_da3_path:
            raise FileNotFoundError(
                f"Variabile d'ambiente COMFY_MODEL_TYPE_DEPTHANYTHING3 non settata"
            )

        model_path = os.path.join(local_da3_path, model)

        print(
            f"\n[DA3] ---------------------------------------------------------------"
        )
        print(f"[DA3] Checking for model...")
        print(f"[DA3] Expected folder: {os.path.abspath(local_da3_path)}")
        print(f"[DA3] Looking for file: {os.path.abspath(model_path)}")

        if os.path.exists(model_path):
            print(f"[DA3] Model FOUND locally. Skipping download.")
            print(
                f"[DA3] ---------------------------------------------------------------\n"
            )
        else:
            print(f"[DA3] ERROR: Model NOT found in {os.path.abspath(model_path)}")
            print(f"[DA3] Download is disabled. Please place the file manually.")
            print(
                f"[DA3] ---------------------------------------------------------------\n"
            )
            raise FileNotFoundError(
                f"Impossibile trovare il modello DA3 in: {model_path}"
            )
        # ---- PHOTON ----
